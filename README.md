...
- **The 3rd strike actually pauses the exam — a human decides, not the heuristic.** The student's exam freezes (timer included, so nobody loses real time waiting) behind an on-screen "waiting for instructor approval" overlay. The instructor's dashboard gets a live "🎥 Proctoring Alerts" card (toast, sound, flash, same as the other live events) with an **Approve & resume** button; clicking it resumes that exact student's exam instantly, wherever they're sitting. The flagged heatmap cell gets a 🎥 badge, and the drill-down drawer shows the snapshot in context. If `RESEND_API_KEY` and `ALERT_TO_EMAIL` are set (see `.env.example`), the instructor also gets a real email — once per exam sitting, not once per strike. After a resume, monitoring restarts fresh — a new pause needs 3 new strikes, not a running total for the whole exam.
