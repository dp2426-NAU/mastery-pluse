...
- **The 3rd strike ends the exam immediately as a hard fail.** No pause, no waiting on anyone — the exam submits right where it stands and is recorded as failed (0%), shown to the student as a plain "❌ Failed — webcam integrity violation" screen. What was actually answered is still preserved underneath and visible to the instructor (nothing is hidden), but the score is forced to 0. The instructor's dashboard gets a live "🎥 Proctoring Alerts" card (toast, sound, flash, same as the other live events) and the flagged heatmap cell gets a 🎥 badge. If `RESEND_API_KEY` and `ALERT_TO_EMAIL` are set (see `.env.example`), the instructor also gets a real email. Worth knowing going in: this is a heuristic acting as the final word, with no human check before the grade is final — the more cautious alternative (end the exam but hold the score for instructor review before it counts) was deliberately not used here.
- **The topic locks after a hard fail — and stays locked until an instructor says otherwise.** This isn't cosmetic: `GET /api/student/exam/:topicKey` actually refuses to hand out the exam for a locked topic (403), and the student's topic card shows "🔒 Locked — ask your instructor" instead of opening it. If a student has a genuine excuse — network dropped, camera glitched, whatever they'd raise with a real professor — the instructor clicks **Grant retake** on that alert (in the Proctoring Alerts panel or the drill-down drawer). That does two real things: the failed attempt's score is excluded from every average from then on (visible in the instructor's drill-down, marked "excluded," never deleted — nothing is hidden), and the topic unlocks live, with the student getting a toast the instant it happens, wherever they're logged in.
- **Honest about its limits.** This is head-pose/eye-closure *approximation*, not eye-tracking — lighting, webcam angle, and glasses all affect it. It's framed everywhere (dashboard copy, email body, this README) as a signal to look closer, never a verdict — the same "detected and logged, not proven" honesty as the rest of the integrity features. A student who declines the camera (or has none) simply continues the exam; the instructor sees no webcam signal for that attempt instead of a hard block. And there's no auto-timeout on a pause: if no instructor is watching, the exam genuinely just stays paused — a real deployment at scale would need an escalation path, which is out of scope here.
